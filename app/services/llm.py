...
from app.mcp.prompts import generate_music
...
class LLMService:
    ...
    async def generate_music_instructions(self, 
                                    description: str,
        ...
        with tempfile.NamedTemporaryFile(mode='w+', suffix='.json', delete=False) as tmp:
            ...
            try:
                ...
                if not os.path.exists(mcp_script_path):
                    ...
                    raise FileNotFoundError(f"MCP script not found at: {mcp_script_path}")
                ...
                if process.returncode != 0:
                    ...
                    logger.error(f"MCP script error (exit code {process.returncode}): {stderr_text}")
                ...
                if not os.path.exists(tmp_path):
                    ...
                    raise FileNotFoundError(f"MCP script did not create output file: {tmp_path}")
                ...
                if result["status"] != "success":
                    logger.error(f"MCP script reported error: {result.get('error', 'Unknown error')}")
                    raise ValueError(f"MCP script error: {result.get('error', 'Unknown error')}")
                
                if "music_description" not in result:
                    logger.error("MCP script output missing 'music_description' field")
                    raise ValueError("MCP script output missing 'music_description' field")
                ...
                for field in required_fields:
                    if field not in music_description:
                        logger.error(f"Music description missing required field: {field}")
                        raise ValueError(f"Music description missing required field: {field}")
